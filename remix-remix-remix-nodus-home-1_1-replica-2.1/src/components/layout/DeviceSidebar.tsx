...
export const DeviceSidebar: React.FC = () => {
  ...
  return (
    <>
      ...
      >
        ...
        <div className="flex-1 overflow-y-auto p-2 space-y-2.5 scrollbar-none flex flex-col items-center">
          {devices.map((device) => {
            ...
            const devColor = DEVICE_COLORS[device.id] || currentAccent.hex;
      ...
      <aside
        className={`fixed top-0 bottom-0 left-0 z-40 w-76 sm:w-84 flex flex-col border-r ${currentTheme.classes.cardBorder} shadow-2xl transition-all duration-300 ease-[cubic-bezier(0.16,1,0.3,1)] will-change-transform ${currentTheme.classes.containerFont} backdrop-blur-3xl ${
        ...
        <div className={`p-3.5 ${currentTheme.classes.cardHeader} flex items-center justify-between`}>
          <div className="flex items-center gap-2.5 overflow-hidden">
            ...
            <div>
              ...
              <p className={`text-[10px] ${currentTheme.classes.textMuted} truncate`}>{devices.length} linked active nodes</p>
            </div>
          </div>
          ...
          <button
            ...
            className={`p-1.5 ${currentTheme.buttonRadius} ${settings.theme === 'material-light' ? 'hover:bg-[#F1F5F9] text-[#64748B] hover:text-[#0F172A]' : 'hover:bg-white/10 text-[#94A3B8] hover:text-[#F1F5F9]'} transition flex items-center justify-center border border-white/5`}
          >
            <ChevronLeft size={16} />
          </button>
        </div>
                ...
                <div className="p-3 space-y-2.5">
                  ...
                  >
                    <div className="flex items-center gap-2.5 min-w-0">
                      <span className={`text-[10px] font-mono font-bold px-1.5 py-0.5 ${currentTheme.buttonRadius} border ${
                        settings.theme === 'material-light' ? 'text-[#475569] bg-[#F1F5F9] border-[#E2E8F0]' : 'text-[#94A3B8] bg-black/40 border-white/5'
                      }`}>
                        0{index + 1}
                      </span>
                      <div
                      ...
                      <div className="min-w-0">
                        ...
                        <p className={`text-[10px] ${currentTheme.classes.textMuted} truncate`}>{device.os}</p>
                      </div>
                    </div>
                    ...
                    <div className="flex items-center gap-0.5 opacity-60 group-hover:opacity-100 transition">
                      <button
                        ...
                        title="Move Up"
                        className={`p-1 rounded ${settings.theme === 'material-light' ? 'hover:bg-[#F1F5F9] text-[#64748B] hover:text-[#0F172A]' : 'hover:bg-white/10 text-[#94A3B8] hover:text-[#F1F5F9]'} disabled:opacity-20`}
                      >
                        <ChevronUp size={14} />
                      </button>
                      <button
                  ...
                  <div
                    onClick={() => selectDevice(device.id)}
                    className={`cursor-pointer grid grid-cols-2 gap-1.5 text-[10px] ${currentTheme.buttonRadius} p-2 border font-mono ${
                      settings.theme === 'material-light' ? 'text-[#334155] bg-[#F8FAFC] border-[#E2E8F0]' : 'text-[#94A3B8] bg-black/30 border-white/[0.06]'
                    }`}
                  >
                  ...
                  {/* Actions Row */}
                  <div className={`flex items-center gap-1.5 pt-1 border-t ${settings.theme === 'material-light' ? 'border-[#E2E8F0]' : 'border-white/[0.06]'}`}>
                      ...
                      <>
                        <button
                          ...
                          className={`flex-1 py-1 px-2 ${currentTheme.buttonRadius} text-[10px] font-bold flex items-center justify-center gap-1 border transition font-mono ${
                            settings.theme === 'material-light' && !isProcOpen ? 'bg-[#F1F5F9] hover:bg-[#E2E8F0] border-[#CBD5E1]' : ''
                          }`}
                          style={{
                            backgroundColor: isProcOpen ? devColor : settings.theme === 'material-light' ? '#F1F5F9' : '#090B10',
                            borderColor: isProcOpen ? devColor : settings.theme === 'material-light' ? '#CBD5E1' : 'rgba(255,255,255,0.1)',
                            color: isProcOpen ? '#FFFFFF' : devColor,
                          }}
                        >
                          <Activity size={11} />
                          <span>Procs</span>
                          <span className={`px-1 rounded text-[9px] font-mono ${settings.theme === 'material-light' ? 'bg-black/10 text-[#0F172A]' : 'bg-black/40'}`}>
                            {procsCount}
                          </span>
                        </button>
                    ...
                    <button
                      ...
                      className={`py-1 px-2 ${currentTheme.buttonRadius} text-[10px] font-bold ${
                        settings.theme === 'material-light'
                          ? 'bg-[#FFFBEB] hover:bg-[#FEF3C7] text-[#D97706] border border-[#FDE68A]'
