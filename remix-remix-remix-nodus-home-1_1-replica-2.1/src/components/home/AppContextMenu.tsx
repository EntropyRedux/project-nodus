...
export const AppContextMenu: React.FC = () => {
  ...
  const subTextClass = isLight ? 'text-[#475569]' : 'text-[#8E8E93]';
  const hoverClass = isLight ? 'hover:bg-black/5' : 'hover:bg-white/10';

  return (
    <div 
      className={`fixed inset-0 z-[120] ${isLight ? 'bg-black/25' : 'bg-black/40'} backdrop-blur-[2px] animate-in fade-in duration-150`}
      ...
      >
        ...
        <div className={`flex items-center gap-2.5 px-2.5 py-2 ${currentTheme.classes.cardHeader}`}>
          ...
          <div className="min-w-0 flex-1">
            <h4 className={`text-xs font-bold truncate ${textClass}`}>
              {currentTheme.archetype === 'hud' ? `[${targetApp.name.toUpperCase()}]` : targetApp.name}
            </h4>
            <p className={`text-[9px] ${subTextClass} truncate`}>{targetApp.packageName || 'System Shortcut'}</p>
          </div>
          <button 
            onClick={closeAppContextMenu}
            className={`${subTextClass} hover:${textClass} p-1 rounded-md transition`}
          >
            <X size={13} />
          </button>
        </div>
        ...
        <div className="space-y-0.5 pt-1">
          <button
            ...
            className={`w-full flex items-center gap-2.5 px-2.5 py-2 ${currentTheme.buttonRadius} text-xs font-semibold ${hoverClass} transition`}
          ...
          <button
            ...
            className={`w-full flex items-center gap-2.5 px-2.5 py-2 ${currentTheme.buttonRadius} text-xs font-medium ${hoverClass} ${textClass} transition`}
          >
          ...
          {targetApp.packageName && (
            <button
              ...
              className={`w-full flex items-center gap-2.5 px-2.5 py-2 ${currentTheme.buttonRadius} text-xs font-medium ${hoverClass} ${subTextClass} hover:${textClass} transition`}
            >
          ...
          <button
            ...
            className={`w-full flex items-center gap-2.5 px-2.5 py-2 ${currentTheme.buttonRadius} text-xs font-medium hover:bg-[#FF3B30]/10 text-[#FF3B30] transition ${isLight ? 'border-t border-[#E2E8F0]' : 'border-t border-white/5'} mt-1`}
          >
