...
import { Folder, Plus, Check, Settings, Sparkles, FolderPlus } from 'lucide-react';
...
export const HomeScreen: React.FC = () => {
  ...
  // Filter apps for current page that are not in dock and not inside a folder
    ...
    >
      ...
      <div className="flex-1 flex flex-col justify-center px-4 py-2 overflow-y-auto no-scrollbar">
        {settings.minimalistMode ? (
          /* Pure Minimalist Text Mode (Clean Minimalist feed) */
          ...
          <div className={`grid ${gridColsClass} gap-y-6 gap-x-2 justify-items-center items-start py-2`}>
            ...
            {currentFolders.map((folder) => (
              ...
              >
                <div className="w-14 h-14 rounded-[1.75rem] bg-[#1C1C1E] border border-white/5 p-2 grid grid-cols-2 gap-1 items-center justify-items-center shadow-xl group-hover:bg-[#2C2C2E] transition-colors">
            ...
            {/* Render direct Home Screen Apps */}
            {currentApps.map((app) => (
