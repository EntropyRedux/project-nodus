...
/**
 * A lightweight, dependency-free SVG QR Code Generator Component.
 * Encodes text value into a 21x21 or 25x25 QR matrix for visual display.
 */
export const QrCode: React.FC<QrCodeProps> = ({
  ...
  // Simple deterministic hash to create matrix pattern for pairing payloads
  const getMatrix = (str: string) => {
    const N = 21;
    const matrix: boolean[][] = Array.from({ length: N }, () => Array(N).fill(false));

    // Helper: Draw Finder Patterns (Top-Left, Top-Right, Bottom-Left 7x7 squares)
    ...
    // Pseudo-random data module placement based on string hash
    let hash = 0;
