...
import { Activity, Zap, Play, Cpu } from 'lucide-react';
...
export const RemoteCanvasWidget: React.FC = () => {
  ...
  const handleExecuteMacro = async (macro: ClusterMacro) => {
    ...
    addNotification({
      ...
      message: `Completed multi-node actions on ${activeDevice.name}.`,
      iconName: macro.icon,
      color: macro.color,
    });
  };
  ...
  return (
    <div className="w-full bg-[#1C1C24]/80 backdrop-blur-xl border border-white/10 rounded-2xl p-3.5 shadow-xl space-y-3">
        ...
        {DEFAULT_CLUSTER_MACROS.map((macro) => {
          ...
          return (
            <button
              ...
              disabled={isRunning}
              className="p-2.5 rounded-xl bg-[#121218] hover:bg-[#22222E] border border-white/5 hover:border-white/20 text-left transition flex items-center justify-between group"
            >
